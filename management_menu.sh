...
# ── Update source ───────────────────────────────
UPDATE_URL="https://codeload.github.com/moryante1/lbpro/zip/refs/heads/main"
UPDATE_FILE_IN_ZIP="lbpro-main/management_menu.sh"
...
# ── Update Loader (in-place replace) ────────────
update_loader() {
    # Resolve the real path of the current script (the loader) -> replaced in place, no folder
    local SELF
    SELF="$(readlink -f "$0" 2>/dev/null)"
    [ -z "$SELF" ] && SELF="$0"

    if [ ! -w "$SELF" ] && [ ! -w "$(dirname "$SELF")" ]; then
        whiptail --title "✗ Update Error" \
            --msgbox "\n  No write permission on:\n  $SELF\n\n  Please run this tool as root." 12 58
        return
    fi

    whiptail --title "⬇  Update Loader" \
        --yesno "\n  The loader will be updated from the official source and\n  replaced in place directly:\n\n  File : $SELF\n\n  ⚠  A backup will be created, then the loader restarts.\n\n  Continue with the update?" \
        16 64 || return

    local TMP_DIR TMP_ZIP TMP_NEW
    TMP_DIR="$(mktemp -d)"
    TMP_ZIP="$TMP_DIR/update.zip"
    TMP_NEW="$TMP_DIR/new_loader.sh"

    show_progress "Updating Loader" "Downloading the latest version..."

    # Download the package
    if ! curl -fsSL "$UPDATE_URL" -o "$TMP_ZIP" 2>/dev/null; then
        rm -rf "$TMP_DIR"
        whiptail --title "✗ Update Failed" \
            --msgbox "\n  Failed to download the update.\n\n  Check your internet connection and the URL." 11 54
        log_action "Loader update failed: download error" "ERROR"
        return
    fi

    # Extract only the loader file from the package
    if ! unzip -o -j "$TMP_ZIP" "$UPDATE_FILE_IN_ZIP" -d "$TMP_DIR" >/dev/null 2>&1; then
        rm -rf "$TMP_DIR"
        whiptail --title "✗ Update Failed" \
            --msgbox "\n  Could not extract the loader file from the package." 10 56
        log_action "Loader update failed: extract error" "ERROR"
        return
    fi
    mv "$TMP_DIR/$(basename "$UPDATE_FILE_IN_ZIP")" "$TMP_NEW"

    # Validate the new file
    if [ ! -s "$TMP_NEW" ] || ! head -1 "$TMP_NEW" | grep -q '^#!/bin/bash'; then
        rm -rf "$TMP_DIR"
        whiptail --title "✗ Update Failed" \
            --msgbox "\n  The downloaded file is not valid.\n\n  Update cancelled." 11 50
        log_action "Loader update failed: invalid file" "ERROR"
        return
    fi
    ...
    if ! bash -n "$TMP_NEW" 2>/dev/null; then
        rm -rf "$TMP_DIR"
        whiptail --title "✗ Update Failed" \
            --msgbox "\n  The new file contains syntax errors.\n\n  Update cancelled to keep the current version." 11 56
        log_action "Loader update failed: syntax check failed" "ERROR"
        return
    fi

    # Backup, then replace directly in the same location
    local BACKUP="${SELF}.bak.$(date '+%Y%m%d%H%M%S')"
    cp -f "$SELF" "$BACKUP" 2>/dev/null

    if ! cat "$TMP_NEW" > "$SELF" 2>/dev/null; then
        rm -rf "$TMP_DIR"
        whiptail --title "✗ Update Failed" \
            --msgbox "\n  Failed to replace the file.\n\n  Backup: $BACKUP" 11 60
        log_action "Loader update failed: write error" "ERROR"
        return
    fi
    chmod +x "$SELF" 2>/dev/null
    rm -rf "$TMP_DIR"

    log_action "Loader updated in place -> $SELF (backup: $BACKUP)" "SUCCESS"

    whiptail --title "✔ Update Complete" \
        --msgbox "\n  ✔  Loader updated successfully!\n\n  File   : $SELF\n  Backup : $BACKUP\n\n  The loader will now restart." 14 64

    # Restart the updated loader in place
    exec bash "$SELF"
}
