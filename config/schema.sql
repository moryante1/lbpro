...
CREATE TABLE IF NOT EXISTS `interfaces` (
  `id`            INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `name`          VARCHAR(30)  NOT NULL UNIQUE COMMENT 'eth0, eth1...',
  ...
  `weight`        TINYINT UNSIGNED  DEFAULT 1 COMMENT 'load balancer weight 1-10',
  `metric`        SMALLINT UNSIGNED DEFAULT 100,
  `is_enabled`    TINYINT(1)   NOT NULL DEFAULT 1,
  `status`        ENUM('up','down','connecting','disabled') DEFAULT 'down',
  `speed_in`      BIGINT UNSIGNED  DEFAULT 0 COMMENT 'bytes rx total',
  `speed_out`     BIGINT UNSIGNED  DEFAULT 0 COMMENT 'bytes tx total',
...
CREATE TABLE IF NOT EXISTS `pppoe_connections` (
  ...
  `persist`         TINYINT(1)   DEFAULT 1 COMMENT 'auto reconnect',
  `maxfail`         TINYINT UNSIGNED  DEFAULT 0 COMMENT '0=unlimited',
...
CREATE TABLE IF NOT EXISTS `vlans` (
  `id`            INT UNSIGNED AUTO_INCREMENT PRIMARY KEY,
  `vlan_id`       SMALLINT UNSIGNED NOT NULL COMMENT '1-4094',
  `name`          VARCHAR(60)  NOT NULL,
  `interface_id`  INT UNSIGNED NOT NULL,
  `vlan_interface` VARCHAR(30) DEFAULT NULL COMMENT 'e.g. eth0.10',
...
-- --------------------------------------------------------
-- Traffic Statistics (per interface per hour)
-- --------------------------------------------------------
...
CREATE TABLE IF NOT EXISTS `api_keys` (
  ...
  `key_prefix`  VARCHAR(12)  NOT NULL COMMENT 'displayed prefix lbpro_xxxxx',
...
-- Default interfaces seed
INSERT INTO `interfaces` (`name`,`display_name`,`type`,`weight`,`status`) VALUES
